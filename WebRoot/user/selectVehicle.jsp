					...
					<table id="sample-table-2"
						...
						<c:forEach items="${resultVehicle.list}" var="vehicle">
							<tr>
                                ...
                                <td class="hidden-480"><span
									class="label label-sm label-warning">${vehicle.state}</span></td>
								<td>
									<div
										class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
										<a class="blue"href="viewVehicle?vid=${vehicle.vid}&&state=user"> <i
											class="icon-zoom-in bigger-130"></i>
										</a> 
									</div>
								</td>
								<td><c:if test="${vehicle.state=='可用'}"><a href="application?vid=${vehicle.vid}">申请车辆</a></c:if>
                        <c:if test="${vehicle.state=='正在使用'}"><a href="getRecord?vid=${vehicle.vid}">查看借阅</a></c:if>
						 </td>
							</tr>
							</c:forEach>
						</tbody>
